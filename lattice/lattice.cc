...
Kagome_Lattice_Cirac_Torus::Kagome_Lattice_Cirac_Torus(const std::array<int,2> &n_uc): Lattice_Base(3,2,n_uc,0,3) 
{
    ...
    for (int sitei=0; sitei<n_sites_total_; sitei++)
    {
        ...
        if (sitei_coord[2]==0)
        {
            neigh_bonds_coord[0]=Coordinate{sitei_coord[0]-1,sitei_coord[1]-1,1};
        ...
        if (sitei_coord[2]==2)
        {
            ...
            neigh_bonds_coord[1]=Coordinate{sitei_coord[0],sitei_coord[1]-1,1};
        }
    ...
    for (int bondi=0; bondi<n_bonds_total_; bondi++)
    {
        ...
        if (bondi_coord[2]==0)
        {
            ...
            neigh_sites_coord[2]=Coordinate{bondi_coord[0],bondi_coord[1],1};
        }
        if (bondi_coord[2]==1)
        {
            neigh_sites_coord[0]=Coordinate{bondi_coord[0]+1,bondi_coord[1]+1,0};
            neigh_sites_coord[1]=Coordinate{bondi_coord[0],bondi_coord[1]+1,2};
            neigh_sites_coord[2]=Coordinate{bondi_coord[0]+1,bondi_coord[1],1};
        }
