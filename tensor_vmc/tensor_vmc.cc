...
template <class TensorT>
TensorT_VMC_WF<TensorT>::TensorT_VMC_WF(const std::vector<int> init_spin_config, const std::vector<TensorT> &combined_tensors, const Lattice_Base &lattice, int maxm):
    spin_config_(init_spin_config),
    combined_tensors_(combined_tensors),
    tensor_rg_(lattice)
{
    std::vector<IndexT> phys_legs;
    for (const auto &tensor: combined_tensors)
    {
        phys_legs.push_back(findtype(tensor,Site));
        if (phys_legs.back().m()!=2)
        {
            cout << "Have not implemented d!=2 case!" << endl;
            exit(1);
        }
    ...
    for (int sitei=0; sitei<spin_config_.size(); sitei++) 
    {
        spin_prod_wf_.push_back(TensorT(dag(phys_legs[sitei](spin_config_[sitei]+1))));
        input_tensors.push_back(combined_tensors_[sitei]*spin_prod_wf_[sitei]);
    }
    tensor_rg_=TensorT_RG<TensorT>(lattice,input_tensors,maxm);
}
template
TensorT_VMC_WF<ITensor>::TensorT_VMC_WF(const std::vector<int> init_spin_config, const std::vector<ITensor> &combined_tensors, const Lattice_Base &lattice, int maxm);
template
TensorT_VMC_WF<IQTensor>::TensorT_VMC_WF(const std::vector<int> init_spin_config, const std::vector<IQTensor> &combined_tensors, const Lattice_Base &lattice, int maxm);
...
void tensor_vmc_parallel(const PEPSt<TensorT> &peps, const Args &measure_args)
{
    ...
    std::string ope=measure_args.getString("Operator","SzSz"),
                init_cond=measure_args.getString("InitSpins","antiferro");
    bool silent=!(measure_args.getBool("SpinFlipPrint",false));
...
void tensor_vmc_parallel(std::vector<TensorT> combined_tensors, const Lattice_Base &lattice, const Args &measure_args)
{
    ...
    int thermal_steps=measure_args.getInt("ThermalSteps",50),
        measure_steps=measure_args.getInt("MeasureSteps",1000),
        sweep_no=combined_tensors.size();
    int maxm=measure_args.getInt("Maxm",100);
    std::string ope=measure_args.getString("Operator","SzSz"),
                init_cond=measure_args.getString("InitSpins","antiferro");
    bool silent=!(measure_args.getBool("SpinFlipPrint",false));
